...
public class RedWarehouseWebcamAutoNoCap2 extends LinearOpMode {
...
//        IntakeUntilTouchSensor, //Drive forward slowly and run the intake until we sense we have a block in our intake
    ...
    double LiftSpeed = 1;
    static double AroundBarriersSidePos = -65.75;

    static double HowfarIntakeGoes = 55;
    static double HowfarIntoWarehouseRobotGoes = 40-1;
    static double WarehouseIncrement = 4;


    static double Angle = -2;

    static double incrment = -(0.375); //was 0.4375, .5
    static double HubIncrement = 0; //was .375
...
//    Boolean hasDetected = false;
    ...
    Pose2d poseEstimate;
    Pose2d startingPosition = new Pose2d(12.5, -61.75, Math.toRadians(270)); //If we Spin First, off by 1.5in
...
//                                .strafeLeft(2)
                                    ...
                                    .splineTo(new Vector2d(-11-4, -41 + AmountofBlocksGotten * HubIncrement), Math.toRadians(-275))
                                    .build();
                        }
...
//                                        .setConstraints(drive.getVelocityConstraint(maxVel / Divider, Math.toRadians(maxAngularVel / Divider), 15.53), drive.getAccelerationConstraint(maxAccel))
                                        .splineToConstantHeading(new Vector2d(HowfarIntakeGoes + WarehouseIncrement, (AroundBarriersSidePos+incrment*2)-1), Math.toRadians(0),
                                                getVelocityConstraint(maxVel/Divider, Math.toRadians(180/Divider), 15.53),getAccelerationConstraint(maxAccel))
//                                .splineTo(new Vector2d(HowfarIntakeGoes,65.5),Math.toRadians(0))
                                        .build();
...
//                                        .setConstraints(drive.getVelocityConstraint(maxVel / Divider, Math.toRadians(maxAngularVel / Divider), 15.53), drive.getAccelerationConstraint(maxAccel))
                                        .splineToConstantHeading(new Vector2d(HowfarIntakeGoes + 2*WarehouseIncrement-2, (AroundBarriersSidePos+incrment*2)-1), Math.toRadians(0),
                                                getVelocityConstraint(maxVel/Divider, Math.toRadians(180/Divider), 15.53),getAccelerationConstraint(maxAccel))
//                                .splineTo(new Vector2d(HowfarIntakeGoes,65.5),Math.toRadians(0))
                                        .build();
                                drive.followTrajectorySequenceAsync(ToWarehouse);
...
//                            }
                                ...
                                Park = drive.trajectorySequenceBuilder(DriveToHub.end())
                                        ...
                                        .splineTo(new Vector2d(HowfarIntoWarehouseRobotGoes+3*WarehouseIncrement,(AroundBarriersSidePos+incrment*4)-1),Math.toRadians(0))
                                        .build();
