...
@Disabled
public class BlueWarehouseThriceAutoGreen extends LinearOpMode {
...
//                    break;
                case TRAJECTORY_1:
                    ...
                    if (!drive.isBusy()) { //Once it gets to blocks detects the block and then starts the lift
                        if (drive.Color.green() > 170) //Needs to be at least >93
                        {
                            level = 3;
                            telemetry.addData("Level:", level);
                            telemetry.addData("Color:", drive.Color.green());
                            telemetry.addData("Color2:", drive.Color2.green());
                            telemetry.update();
                        } else if (drive.Color2.green() > 100) //Needs to be at least >76
                        {
                            level = 2;
                            telemetry.addData("Level:", level);
                            telemetry.addData("Color:", drive.Color.green());
                    ...
                    if (!drive.isBusy()) {
                        ...
                        if(AmountofBlocksGotten>1)
                        {
                            TrajectorySequence trajectory3_2 = drive.trajectorySequenceBuilder(trajectory2.end())
                                    .splineToLinearHeading(new Pose2d(8.5,65.75, Math.toRadians(3)),Math.toRadians(3)) //At Barrier, off by 1.5 in in y
